  ...
  useEffect(() => {
    ...
    fetch("https://sufian111.github.io/travel-guru-api/data.json")
      ...
      .then((data) => {
        ...
        if (data) {
          let getCurrentData = data[sliderCurrentId];
        ...
        <Grid container spacing={0}>
          <Grid item lg={5} xs={12}>
            <div className={classes.root}>
              ...
              >
                {currentData
                  ? currentData.description.substr(0, 200) + "..."
                  : "loading"}
              </Typography>
              {currentData ? (
                <Link to={`/booking-form/${currentData.id}`}>
                  <Button className={classes.btnStyle} color="inherit">
                    Booking{" "}
                    <ArrowForwardIcon
            ...
            <div className={classes.sliderRoot}>
              <Slider {...settings}>
                {sliderData &&
                  sliderData.map((v) => {
                    return (
                      <div key={v.id} className="slider-item">
